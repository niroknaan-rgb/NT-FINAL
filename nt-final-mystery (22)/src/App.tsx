                ...
                {(isBoxUnlocked || isLidFalling) && (
                  <div className="absolute inset-2 bg-[#1a130d] border border-[#2b1f15] rounded shadow-inner flex items-center justify-center overflow-hidden">
                    {!hasItem("recipe-book") && !pickedUp && (
                      <motion.div
                        className="w-[85%] h-[85%] cursor-pointer hover:scale-110 active:scale-95 transition-transform duration-100 flex items-center justify-center z-10"
                        onClick={(e) => {
                          e.stopPropagation();
...
const parseHebrewDateTime = (dateTimeStr: string): number => {
  if (!dateTimeStr) return 0;
  try {
    const clean = dateTimeStr.replace(/\./g, "/").replace(/,/g, " ").replace(/\s+/g, " ").trim();
    ...
    if (dateSubparts.length < 3) return 0;
    ...
    console.error("Error parsing Hebrew date-time string:", dateTimeStr, e);
    return 0;
  }
};

const enrichParticipantsWithTicketNumbers = (list: Participant[]): Participant[] => {
  const sortedChronologically = [...list].sort((a, b) => {
    const tA = parseHebrewDateTime(a.createdAt || a.timestamp || "");
    const tB = parseHebrewDateTime(b.createdAt || b.timestamp || "");
    return tA - tB;
  });
  
  const mapIdToTicketNumber: Record<string, number> = {};
  sortedChronologically.forEach((p, idx) => {
    const key = p.id || `${p.email}-${p.phone}`;
    mapIdToTicketNumber[key] = idx + 1;
  });
  ...
  return list.map((p) => {
    ...
    return {
      ...p,
      ticketNumber: mapIdToTicketNumber[key] || 1
    };
  });
};
...
const ParticipantsTableView = ({ onClose }: { onClose: () => void }) => {
  ...
  const [sortBy, setSortBy] = useState<"lastName" | "ticketNumber" | "default">("default");
  ...
  const enrichedList = React.useMemo(() => {
    return enrichParticipantsWithTicketNumbers(participants);
  }, [participants]);

  const sortedParticipants = React.useMemo(() => {
    const listCopy = [...enrichedList];
    if (sortBy === "lastName") {
      return listCopy.sort((a, b) => {
        const nameA = a.lastName || "";
        const nameB = b.lastName || "";
        return nameA.localeCompare(nameB, "he-IL");
      });
    } else if (sortBy === "ticketNumber") {
      return listCopy.sort((a, b) => (a.ticketNumber || 0) - (b.ticketNumber || 0));
    }
    return listCopy;
  }, [enrichedList, sortBy]);
      ...
      >
        <div className="p-6 border-b border-slate-800 flex items-center justify-between flex-wrap gap-4">
          <div>
            ...
            {/* Sorting controls directly in the header section */}
            <div className="flex items-center gap-2 mt-3 p-1 bg-slate-950/40 rounded-xl border border-slate-800/80 max-w-max">
              <span className="text-xs text-slate-400 px-2 font-medium">מיון לפי:</span>
              <button
              ...
              <button
                onClick={() => setSortBy("ticketNumber")}
                className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${
                  sortBy === "ticketNumber"
                    ? "bg-red-600 text-white shadow-md shadow-red-600/10"
                    : "text-slate-355 hover:bg-slate-800 hover:text-white"
              ...
              <table className="w-full text-right border-collapse text-sm">
                <thead>
                  <tr className="bg-slate-950 text-slate-400 border-b border-slate-800 font-bold">
                    ...
                    <th className="p-4 border-l border-slate-800 text-center w-28">מספר כרטיס</th>
                ...
                <tbody className="divide-y divide-slate-800 text-slate-300">
                  {sortedParticipants.map((person, idx) => {
                    const isSelected = selectedParticipantId === person.id;
                    return (
                      <tr 
                        key={person.id || idx} 
                        onClick={() => setSelectedParticipantId(isSelected ? null : (person.id || null))}
                        className={`cursor-pointer transition-colors ${isSelected ? "bg-red-950/40 hover:bg-slate-800/40" : "hover:bg-slate-800/30 text-slate-300"}`}
                      >
                        <td className="p-4 border-l border-slate-800 text-center">
                          <div className={`w-4 h-4 mx-auto rounded-full border flex items-center justify-center transition-all ${isSelected ? "border-red-500 bg-red-600" : "border-slate-600 bg-transparent"}`}>
                            {isSelected && <div className="w-1.5 h-1.5 rounded-full bg-white"></div>}
                          </div>
                        </td>
                        <td className="p-4 border-l border-slate-800 text-center font-mono font-bold text-red-400">
                          #{person.ticketNumber?.toString().padStart(3, '0')}
                        </td>
          ...
          <AnimatePresence>
            {showBedroomKeyReveal && (
              <motion.div
                ...
                onClick={() => {
                  setInventoryItems((prev) => {
                    if (!prev.includes("bedroom-key")) {
                      return ["bedroom-key", ...prev];
                    }
                    return prev;
                  });
              ...
              >
                ...
                <motion.img
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    setInventoryItems((prev) => {
                      if (!prev.includes("uv-flashlight")) {
                        return ["uv-flashlight", ...prev];
                      }
                      return prev;
                    });
