...
import { initializeFirestore, collection, addDoc, getDocs, deleteDoc, doc, onSnapshot, getDocFromServer } from "firebase/firestore";
...
const CloseUpContainer = ({
  ...
  return (
    ...
    >
      <div
        className="relative flex flex-col items-center justify-center cursor-default bg-black/40 rounded-lg shadow-2xl overflow-hidden"
        onClick={(e) => e.stopPropagation()}
        style={{
          width: "100%",
          height: "100%",
          maxWidth: `min(calc(100vw - 2rem), calc(85vh * ${aspectRatio}))`,
          maxHeight: `min(calc(100vh - 2rem), calc(85vw / ${aspectRatio}))`,
          containerType: "size",
        }}
      >
        {children}
  ...
  if (characterType === "inspector") {
    ...
    dialogues = hasAgreedToHelpPianist
      ...
      : [
          ...
          "אם תחזירו לי אותם אולי אצליח להתרכז\nבמה שקרה בשעת הרצח",
    ...
    dialogues = hasAgreedToHelpCaretaker
      ...
      : [
          ...
          "ניר הביא לי את הנר זה...\nתמיד הוא היה מלא באהבה",
          "אחד מהאחרים גנב לי אותו! אני בטוח!",
          "הם תמיד קינאו בקשר שלי ושל ניר",
          "אם תמצאו לי את הנר\nאני ממש אשמח לסייע לכם קצת",
          "", // Step 7: Choices
          choice === 1
            ? "תודה! ידעתי שתסכימו!"
    ...
    dialogues = [
      ...
      "תעזרו לי לגלות עוד על הרצח שקרה פה\nולגלות מי הרוצח",
        ...
        <div className="absolute inset-0 border-8 border-[#4d6b4d] bg-[#1f3a1f] flex flex-col items-center justify-center p-8">
          ...
          {index === 0 && isOpen && (
            <div className="relative w-full h-[95%] flex items-center justify-center z-10">
              {/* Wooden or Metal Locked Box Container - Perfect Square & Larger (Fixed and enlarged on mobile phones) */}
              <div 
                onClick={handleBoxClick}
                className={`relative w-[320px] h-[320px] max-w-[95%] max-h-[95%] rounded-xl border-4 border-[#3c2a1a] shadow-2xl flex items-center justify-center transition-all overflow-hidden ${
              ...
              >
                ...
                {(isBoxUnlocked || isLidFalling) && (
                  <div className="absolute inset-4 bg-[#1a130d] border border-[#2b1f15] rounded-lg shadow-inner flex items-center justify-center overflow-hidden">
                    {!hasItem("recipe-book") && !pickedUp && (
                      <motion.div
                        className="w-[80%] h-[80%] cursor-pointer hover:scale-110 active:scale-95 transition-transform duration-100 flex items-center justify-center z-10"
                        onClick={(e) => {
                          e.stopPropagation();
                ...
                {!isBoxUnlocked && (
                  <motion.div
                    ...
                    className={`absolute inset-0 bg-[#2b1f15] rounded-xl flex flex-col items-center justify-center p-4 z-20 ${
...
const BathroomMirrorCloseUp = ({ onClose }: { onClose: () => void }) => {
  ...
  return (
    <CloseUpContainer onClose={onClose} aspectRatio={3 / 4}>
      <div className="w-full h-full relative flex items-center justify-center">
        ...
        <div className="w-[60%] h-[80%] bg-[#1e272e] border-4 border-[#808e9b] rounded-t-full relative flex flex-col items-center pt-[15%] gap-[8%] shadow-inner z-10">
          ...
          <div className="mt-auto mb-4 w-[85%] h-20 relative flex items-end">
            <img
              src="https://i.postimg.cc/vT90RC4Z/qwd-X-2.png"
              alt="Clue"
              className="absolute left-[5%] bottom-0 w-20 h-auto object-contain"
              referrerPolicy="no-referrer"
            />

            {/* Box covering the image partially */}
            <motion.div
              className="absolute left-[5%] bottom-0 w-16 h-16 bg-[#8b5a2b] border-2 border-[#5c3a18] cursor-pointer z-10 flex items-center justify-center rounded-sm"
        ...
        {/* The Mirror Door */}
        <motion.div
          className="absolute w-[60%] h-[80%] border-4 border-[#808e9b] bg-[#d2dae2] rounded-t-full shadow-[0_0_30px_rgba(210,218,226,0.2)] origin-left cursor-pointer z-20 overflow-hidden"
          onClick={() => setIsOpen(!isOpen)}
          animate={{
            rotateY: isOpen ? -110 : 0,
          }}
        ...
        >
          ...
          {/* Handle */}
          <div className="absolute top-1/2 right-[5%] w-2.5 h-10 bg-[#485460] rounded-full transform -translate-y-1/2"></div>
        </motion.div>
      </div>
    </CloseUpContainer>
  );
        ...
        >
          {isSheetMusicPlaced ? (
            <img
              ...
              className="absolute bottom-[2px] w-[150%] h-[160%] object-contain drop-shadow-2xl z-10 pointer-events-none origin-bottom scale-135"
              alt="Sheet Music"
              referrerPolicy="no-referrer"
            />
          ) : (
...
const InventoryToolbar = ({
  ...
  const updateConstraints = () => {
    if (containerRef.current && contentRef.current) {
      ...
      const innerContainerWidth = containerWidth - paddingLeft - paddingRight - borderLeft - borderRight;

      if (contentWidth > containerWidth) {
        // In RTL, we start at 0 (right side) and drag right (positive) to see left items
        // We stop exactly when the left edge of content matches the inner left edge of container
        setDragConstraints({ left: 0, right: contentWidth - innerContainerWidth });
      } else {
  ...
  return (
    <div className="absolute bottom-6 right-6 z-[200] flex items-center gap-2">
      <button
        onClick={onToggle}
        className={`p-3 sm:p-4 border-2 border-[#3a3f47] transition-all rounded-full shadow-xl z-10 flex-shrink-0 ${isOpen ? "bg-[#2a2d34] text-white" : "bg-[#1a1c20] text-gray-300 hover:bg-[#2a2d34] hover:text-white"}`}
        style={{
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            style={{
              originX: 1,
              maxWidth: "min(440px, calc(100vw - 6rem))",
              width: "max-content",
            }}
          >
            <div
...
const ResetGameButton = ({ onReset }: { onReset: () => void }) => {
  ...
  return (
    <div className="absolute top-4 left-4 z-[99999] flex items-center gap-1.5 pointer-events-auto select-none" dir="rtl">
      {confirmOpen && (
        <motion.div
          initial={{ opacity: 0, x: -8 }}
          animate={{ opacity: 1, x: 0 }}
          exit={{ opacity: 0, x: -8 }}
          className="bg-red-950/90 backdrop-blur-md border border-red-800/30 text-red-200 text-[11px] font-sans font-medium px-2.5 py-1.5 rounded-lg shadow-md flex items-center gap-1 cursor-pointer hover:bg-red-900/80 transition-colors"
          onClick={onReset}
        >
          <span>לחץ לאיפוס</span>
        </motion.div>
      )}
      <button
        onClick={() => {
        ...
        className={`w-7 h-7 sm:w-8 sm:h-8 transition-all duration-300 z-[99999] border rounded-lg flex items-center justify-center cursor-pointer ${
          confirmOpen 
            ? "bg-red-950/95 text-red-200 border-red-800/40 shadow-sm" 
            : "bg-black/10 hover:bg-black/25 border-white/5 text-white/30 hover:text-white/60 hover:border-white/10"
        }`}
        title="אפשרויות"
      >
