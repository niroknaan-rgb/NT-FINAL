...
const MobileWarningScreen = ({ onAcknowledge }: { onAcknowledge: () => void }) => {
  return (
    <div className="absolute inset-0 z-[100000] flex flex-col items-center justify-center bg-black/85 p-6 text-center backdrop-blur-md" dir="rtl">
      <div className="bg-[#1c1c1e]/95 border border-red-500/40 rounded-3xl p-6 max-w-sm w-full flex flex-col items-center shadow-2xl relative">
        <AlertCircle className="w-14 h-14 flex-shrink-0 text-red-500 mb-4" />
        <p className="text-[4.2cqw] md:text-xl font-bold mb-2 font-sans text-zinc-100">האפליקציה נועדה למשחק בטלפון</p>
        <p className="text-[3.8cqw] md:text-base font-medium mb-2 font-sans text-red-400">אם אתם משתמשים במחשב המשחק לא יעבוד</p>
        <p className="text-[3.5cqw] md:text-sm font-light mb-6 font-sans text-zinc-400">אנא השתמשו במכשיר סלולרי</p>
        <button 
          onClick={onAcknowledge}
          className="px-6 py-2.5 bg-white text-black font-extrabold rounded-full hover:bg-gray-200 active:scale-95 transition-all text-sm font-sans shadow-[0_0_15px_rgba(255,255,255,0.25)] flex-shrink-0"
        >
          המשך למשחק
        </button>
      </div>
    ...
    >
      <div
        ...
        style={{
          width: "100%",
          height: "100%",
          maxWidth: `min(calc(100% - 2rem), calc(90cqh * ${aspectRatio}))`,
          maxHeight: `min(calc(100% - 2rem), calc(90cqw / ${aspectRatio}))`,
        }}
      >
        {children}
      </div>
...
const CharacterInteractionOverlay = ({
  ...
  if (characterType === "inspector") {
    ...
    dialogues = [
      "",
      "הייתי עסוק עם המכונית, עבדתי עליה לקראת חצות",
    ...
    dialogues = ["", "יופי! אם תוכלו להכין משהו זה יהיה נהדר"];
    ...
    if (step >= 6) {
      dialogues = [
        ...
        "אם תביאו לי אספר לכם\nעל כל מה שאני יודע מהלילה של הרצח", // step 9
      ];
      showChoices = false;
      ...
      dialogues = hasAgreedToHelpPainter
        ? [
            "",
            "מצאתם כבר את הצבע שלי?\nאני צריך כחול! מהר! לפני שבורח לי הרעיון!",
          ]
        : [
            "",
    ...
    dialogues = hasAgreedToHelpPianist
      ? ["", "לה לה לה.. מצאתם את התווים שלי?\nעוד שניה יש לי את זה..\nלה לה לה.. אוף איך זה הלך?"]
      : [
          "",
    ...
    dialogues = [
      ...
      "קראתי לכולם לבוא לראות והסכמנו\nשצריך לקרוא למישהו.. אז קראנו לליצן..",
    ...
    dialogues = [
      ...
      "ואז שתיקה פתאומית, באמצע המוזיקה.\nלא סיים, פשוט הפסיק.",
      "ואז פתאום צעקה מהסלון,\nיצאתי לראות מה קרה,",
    ...
    >
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
        ...
        <div className="relative flex items-center justify-center w-full h-[60cqh] pointer-events-auto cursor-default">
          ...
          {item === "sheet-music" && (
            <div className="w-[58cqw] h-[75cqh] flex items-center justify-center">
              <img
                src="https://i.postimg.cc/h4xJxzRJ/twwym.png"
                className="w-[110%] h-[110%] max-w-full max-h-full object-contain drop-shadow-xl"
                alt="Sheet Music"
                referrerPolicy="no-referrer"
              />
            </div>
